...
(5,'Fahim Chowdhury','Dhaka');  -- has no orders (useful for LEFT JOIN)
...
(9001,5001,2200.00,'2025-11-01','Card'),   -- 2*800 + 5*120 = 2200
(9002,5002,4280.00,'2025-11-05','BKash');  -- 3500 + 780 = 4280
...
PRINT ''
PRINT ''
PRINT '-- Filtering (WHERE) + Sorting (ORDER BY)'
...
PRINT ''
PRINT ''
PRINT '-- Filtering with multiple conditions (AND/OR)'
...
SELECT OrderID, FullName, ProductName, Qty, UnitPrice, LineTotal
FROM vOrderLineDetails
...
SELECT
  c.CustomerID, c.FullName, c.City,
  oh.OrderID, oh.OrderDate, oh.Status
FROM Customer c
LEFT JOIN OrderHeader oh ON oh.CustomerID = c.CustomerID
ORDER BY c.CustomerID, oh.OrderID;

PRINT ''
PRINT ''
PRINT '-- LEFT JOIN + filtering to find "no match" rows'
...
ORDER BY p.ProductID, oi.OrderID;

PRINT ''
PRINT ''
PRINT '-- RIGHT JOIN + filter to find unsold products'
...
SELECT
  ...
  pay.Method,
  CASE
    WHEN oh.OrderID IS NULL THEN 'Payment has no order (data issue)'
    WHEN pay.OrderID IS NULL THEN 'Order not paid yet'
    ELSE 'Order has payment'
  END AS MatchNote
FROM OrderHeader oh
...
FROM Product p
CROSS JOIN (VALUES (0),(5),(10),(15)) d(DiscountPct)
ORDER BY p.ProductName, d.DiscountPct;
...
PRINT ''
PRINT ''
PRINT '-- Aggregate per ORDER (total value of each order)'
...
PRINT ''
PRINT ''
PRINT '-- Aggregate per CUSTOMER (how much each customer spent)'
...
FROM vOrderLineDetails
WHERE OrderStatus IN ('Placed','Shipped')
GROUP BY CustomerID, FullName
HAVING SUM(LineTotal) >= 500      -- only show customers who spent at least 500
...
SELECT * FROM OrderItem WHERE OrderID = 5001;
SELECT * FROM Payment   WHERE OrderID = 5001;

-- Delete the parent row (OrderHeader)
DELETE FROM OrderHeader WHERE OrderID = 5001;
...
SELECT * FROM OrderHeader WHERE OrderID = 5001;
